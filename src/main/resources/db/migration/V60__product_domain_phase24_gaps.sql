...
ALTER TABLE products
ADD CONSTRAINT chk_purchase_price_positive CHECK (
...
CREATE OR REPLACE FUNCTION products_search_vector_update() RETURNS trigger AS $$ BEGIN new.search_vector := to_tsvector(
        'english',
        coalesce(new.name, '') || ' ' || coalesce(new.description, '') || ' ' || coalesce(new.sku, '') || ' ' || coalesce(new.barcode, '')
    );
RETURN new;
...
SET search_vector = to_tsvector(
        'english',
        coalesce(name, '') || ' ' || coalesce(description, '') || ' ' || coalesce(sku, '') || ' ' || coalesce(barcode, '')
    );
...
CREATE INDEX IF NOT EXISTS idx_products_tenant_active_listing ON products (tenant_id, is_deleted, id)
