...
) THEN DROP TABLE stock_movements CASCADE;
END IF;
IF EXISTS (
    SELECT 1
    FROM pg_partitioned_table p
...
) THEN DROP TABLE audit_logs CASCADE;
END IF;
END $$;
...
AND NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'stock_movements_old'
) THEN
...
IF EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'stock_movements_id_seq'
) THEN ALTER SEQUENCE stock_movements_id_seq OWNED BY NONE;
END IF;
END IF;
IF EXISTS (
    SELECT 1
...
) THEN ALTER SEQUENCE audit_logs_id_seq OWNED BY NONE;
END IF;
END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS stock_movements_p2024 PARTITION OF stock_movements FOR
VALUES
FROM ('2024-01-01') TO ('2025-01-01');
CREATE TABLE IF NOT EXISTS stock_movements_p2025 PARTITION OF stock_movements FOR
VALUES
FROM ('2025-01-01') TO ('2026-01-01');
CREATE TABLE IF NOT EXISTS stock_movements_p2026 PARTITION OF stock_movements FOR
VALUES
...
ALTER TABLE stock_movements
ALTER COLUMN id
SET DEFAULT nextval('stock_movements_id_seq');
...
CREATE TABLE IF NOT EXISTS audit_logs_p2024 PARTITION OF audit_logs FOR
VALUES
FROM ('2024-01-01') TO ('2025-01-01');
CREATE TABLE IF NOT EXISTS audit_logs_p2025 PARTITION OF audit_logs FOR
VALUES
FROM ('2025-01-01') TO ('2026-01-01');
CREATE TABLE IF NOT EXISTS audit_logs_p2026 PARTITION OF audit_logs FOR
VALUES
FROM ('2026-01-01') TO ('2027-01-01');
...
ALTER TABLE audit_logs
ALTER COLUMN id
SET DEFAULT nextval('audit_logs_id_seq');
ALTER SEQUENCE audit_logs_id_seq OWNED BY audit_logs.id;
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_class WHERE relname = 'audit_logs_old'
    ) THEN

        PERFORM setval(
            'audit_logs_id_seq',
            COALESCE((SELECT MAX(id) FROM audit_logs_old), 1)
        );
...
ALTER TABLE stock_movements
ADD CONSTRAINT fk_sm_user FOREIGN KEY (created_by) REFERENCES users(id);
