...
ON alerts (tenant_id, id);

-- 3. Ensure foreign key constraints exist (add if missing)
DO $$
BEGIN
    -- Ensure unique constraint on referenced columns for composite FK
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'ux_customers_tenant_id'
    ) THEN
        ALTER TABLE customers ADD CONSTRAINT ux_customers_tenant_id UNIQUE (tenant_id, id);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'ux_suppliers_tenant_id'
    ) THEN
        ALTER TABLE suppliers ADD CONSTRAINT ux_suppliers_tenant_id UNIQUE (tenant_id, id);
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'fk_orders_supplier' AND table_name = 'orders'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'fk_inventory_product' AND table_name = 'inventory'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE customers ADD CONSTRAINT chk_customers_is_deleted
        CHECK (is_deleted IN (TRUE, FALSE));
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE suppliers ADD CONSTRAINT chk_suppliers_is_deleted
        CHECK (is_deleted IN (TRUE, FALSE));
    END IF;
END
$$;
