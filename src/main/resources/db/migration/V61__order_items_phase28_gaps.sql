...
ALTER TABLE order_items
    RENAME COLUMN total TO subtotal;
...
ALTER TABLE order_items
ADD CONSTRAINT chk_order_items_discount_positive CHECK (discount >= 0);
-- 5. CHECK constraint: tax_rate must be >= 0
ALTER TABLE order_items
ADD CONSTRAINT chk_order_items_tax_rate_positive CHECK (tax_rate >= 0);
...
DO $$ BEGIN IF NOT EXISTS (
    ...
    WHERE constraint_name = 'fk_order_items_tenant'
