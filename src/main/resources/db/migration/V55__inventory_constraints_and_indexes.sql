...
DO $$ BEGIN IF EXISTS (
    ...
    WHERE table_name = 'inventory'
        AND constraint_name = 'chk_inventory_non_negative'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'uk_inventory_tenant_product'
) THEN
ALTER TABLE inventory
ADD CONSTRAINT uk_inventory_tenant_product UNIQUE (tenant_id, product_id);
END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_inventory_low_stock ON inventory (tenant_id, low_stock_threshold)
WHERE low_stock_threshold IS NOT NULL;
-- 5. Add index for reorder level queries
CREATE INDEX IF NOT EXISTS idx_inventory_reorder_level ON inventory (tenant_id, reorder_level)
