...
public class InvoiceService {
  ...
  public InvoiceService(
    ...
    this.invoiceGeneratedCounter =
        Counter.builder("invoice.generated.total").register(meterRegistry);
    this.invoiceVoidedCounter = Counter.builder("invoice.voided.total").register(meterRegistry);
    this.pdfGenerationFailureCounter =
        Counter.builder("invoice.pdf_generation.failures").register(meterRegistry);
    this.duplicateInvoiceAttemptCounter =
        Counter.builder("invoice.duplicate_attempts").register(meterRegistry);
    this.invoiceGenerationTimer =
        Timer.builder("invoice.generation.latency").register(meterRegistry);
    this.pdfGenerationTimer =
        Timer.builder("invoice.pdf_generation.latency").register(meterRegistry);
  }
    ...
    return pdfGenerationTimer.record(
        () -> {
          try {
                          ...
                          Product product =
                              productRepository
                                  .findByIdAndTenantIdAndIsDeletedFalse(
                                      Objects.requireNonNull(item.getProductId()), tenantId)
                                  .orElseThrow(
            ...
            context.setVariable(
                "tenantGstin", tenant.getGstin() != null ? tenant.getGstin() : "GSTIN-TBD");
  ...
  public Page<Invoice> getOverdueInvoices(Pageable pageable) {
    Long tenantId = TenantContext.getTenantId();
    return invoiceRepository.findByTenantIdAndStatusNotAndDueDateBefore(
        tenantId, "PAID", LocalDate.now(), pageable);
  }
