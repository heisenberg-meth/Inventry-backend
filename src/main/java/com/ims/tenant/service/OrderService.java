...
public class OrderService {
    ...
    @Transactional
    public Map<String, Object> createPurchaseOrder(Long tenantId, Map<String, Object> request, Long userId) {
        ...
        supplierRepository.findByIdAndTenantId(supplierId, tenantId)
        ...
        for (Map<String, Object> item : items) {
            ...
            BigDecimal discount = item.containsKey("discount") ? new BigDecimal(item.get("discount").toString()) : BigDecimal.ZERO;
            BigDecimal taxRate = item.containsKey("tax_rate") ? new BigDecimal(item.get("tax_rate").toString()) : BigDecimal.ZERO;

            BigDecimal itemTotal = unitPrice.multiply(BigDecimal.valueOf(qty)).subtract(discount);
            BigDecimal itemTax = itemTotal.multiply(taxRate).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
        ...
        log.info("Purchase order created: id={} tenant={} total={}", order.getId(), tenantId, totalAmount);
        return Map.of("order_id", order.getId(), "total", totalAmount);
    }

    @Transactional
    public Map<String, Object> createSalesOrder(Long tenantId, Map<String, Object> request, Long userId) {
        Long customerId = request.containsKey("customer_id") ? Long.valueOf(request.get("customer_id").toString()) : null;
        List<Map<String, Object>> items = (List<Map<String, Object>>) request.get("items");
        String notes = request.getOrDefault("notes", "").toString();
        ...
        log.info("Sales order created: id={} tenant={} total={} invoice={}", order.getId(), tenantId, totalAmount, invoice.getInvoiceNumber());
        return Map.of(
