...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
  ...
  @Query(
      "SELECT p FROM Payment p JOIN Invoice i ON p.invoiceId = i.id JOIN Order o ON i.orderId = o.id WHERE o.customerId = :customerId")
  java.util.List<Payment> findByCustomerId(@Param("customerId") Long customerId);

  @Query(
      "SELECT p FROM Payment p JOIN Invoice i ON p.invoiceId = i.id JOIN Order o ON i.orderId = o.id WHERE o.supplierId = :supplierId")
