...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
  ...
  Optional<Inventory> findByProductIdAndTenantId(@Param("productId") Long productId, @Param("tenantId") Long tenantId);
  ...
  @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.quantity <= i.lowStockThreshold")
  ...
  Page<Inventory> findBelowReorderLevelByTenantId(@Param("tenantId") Long tenantId, Pageable pageable);

  @Query("SELECT CASE WHEN COUNT(i) > 0 THEN true ELSE false END FROM Inventory i WHERE i.productId = :productId AND i.tenantId = :tenantId")
  boolean existsByProductIdAndTenantId(@Param("productId") Long productId, @Param("tenantId") Long tenantId);
}