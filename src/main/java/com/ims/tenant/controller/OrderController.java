...
public class OrderController {
    ...
    public ResponseEntity<Page<Order>> getOrders(@RequestParam(required = false) String type, Pageable pageable) {
        if (type != null) {
    ...
    public ResponseEntity<Map<String, Object>> createPurchaseOrder(@RequestBody Map<String, Object> request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(orderService.createPurchaseOrder(tenantId, request, userId));
    }

    @PostMapping("/sale")
    @RequiresRole({"ADMIN", "MANAGER", "STAFF"})
    @Operation(summary = "Create sale order")
    public ResponseEntity<Map<String, Object>> createSalesOrder(@RequestBody Map<String, Object> request) {
        Long tenantId = TenantContext.get();
        Long userId = (Long) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return ResponseEntity.status(HttpStatus.CREATED).body(orderService.createSalesOrder(tenantId, request, userId));
    }
