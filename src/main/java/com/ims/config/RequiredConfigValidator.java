...
public class RequiredConfigValidator {
    ...
    public void validateRequiredConfigs() {
        ...
        if (isProduction && !missingConfigs.isEmpty()) {
            log.error("FATAL: Production startup aborted. Missing required configuration: {}", missingConfigs);
            throw new IllegalStateException(
                    "Required environment variables not configured for production: " + missingConfigs);
        }

        if (!missingConfigs.isEmpty()) {
            log.warn("WARNING: Some required configurations are using placeholder values: {}. " +
                    "These MUST be configured before production deployment.", missingConfigs);
        } else {
