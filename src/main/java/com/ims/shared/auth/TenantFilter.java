 ...
 * Authoritative tenant resolution for authenticated routes is handled by JwtFilter.
 */
@Slf4j
@Component
public class TenantFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(@NonNull HttpServletRequest request) {
        ...
        // 2. ONLY apply for specific public routes that legitimately need header-based tenant resolution.
        // For all other routes (authenticated), JwtFilter will set the authoritative tenant ID.
        // If we don't have specific public multi-tenant routes yet, we can skip filtering for all
        // EXCEPT those starting with specific prefixes if needed.
        
        // As per code review recommendation, we only apply this for truly public multi-tenant routes.
