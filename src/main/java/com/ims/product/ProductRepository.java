...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Modifying
    @Query("UPDATE Product p SET p.stock = p.stock - :qty, p.updatedAt = :now WHERE p.id = :productId AND p.stock >= :qty")
    int decrementStockIfAvailable(
            @Param("productId") Long productId, @Param("qty") int qty, @Param("now") LocalDateTime now);

    @Modifying
    @Query("UPDATE Product p SET p.stock = p.stock + :qty, p.updatedAt = :now WHERE p.id = :productId AND (p.stock + :qty) >= 0")
