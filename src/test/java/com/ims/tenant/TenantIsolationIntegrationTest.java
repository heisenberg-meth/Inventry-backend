...
class TenantIsolationIntegrationTest extends BaseIntegrationTest {
  ...
  void setup() throws Exception {
    ...
    // User for tenant 1
    jdbcTemplate.update(
        "INSERT INTO users (name, email, password_hash, role, scope, tenant_id, "
  ...
  private String loginAndGetToken(String email, String password, String companyCode)
    ...
    MockHttpServletRequestBuilder loginBuilder =
        post("/api/auth/login")
  ...
  void tenant1CannotAccessTenant2Data() throws Exception {
    ...
    MockHttpServletRequestBuilder createProductBuilder =
    ...
    MockHttpServletRequestBuilder getProductsBuilder =
        get("/api/v1/tenant/products").header("Authorization", "Bearer " + tenant2Token);
    mockMvc
  ...
  void tenantContextIsIsolated() throws Exception {
    ...
    MockHttpServletRequestBuilder getTenant1Builder =
        get("/api/tenant/settings").header("Authorization", "Bearer " + tenant1Token);
    mockMvc
        .perform(getTenant1Builder)
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.workspace_slug").value("t1"));

    MockHttpServletRequestBuilder getTenant2Builder =
        get("/api/tenant/settings").header("Authorization", "Bearer " + tenant2Token);
    mockMvc
