...
public class AuditTrailIntegrationTest extends BaseIntegrationTest {
        ...
        void testProductAuditLogging() throws Exception {
                ...
                SignupResponse response = signupService.signup(Objects.requireNonNull(signup));
                verifyUserEmail("admin@audit.com");
                verifyUser("admin@audit.com");

                Long tenantId = Objects
                                .requireNonNull(tenantRepository.findByWorkspaceSlug("audit-corp").orElseThrow()
                                                .getId());
                String token = login("admin@audit.com", "password123",
        ...
        void testAuditIsolation() throws Exception {
                // Tenant 1
                SignupResponse r1 = signupService.signup(
                                Objects.requireNonNull(createSignupRequest("Audit Corp 1", "audit-corp-1",
                                                "admin1@audit.com")));
                verifyUserEmail("admin1@audit.com");
                verifyUser("admin1@audit.com");
                Long t1Id = Objects.requireNonNull(
                                tenantRepository.findByWorkspaceSlug("audit-corp-1").orElseThrow().getId());
                String t1Token = login("admin1@audit.com", "password123", Objects.requireNonNull(r1.getCompanyCode()),
                                t1Id);

                // Tenant 2
                SignupResponse r2 = signupService.signup(
                                Objects.requireNonNull(createSignupRequest("Audit Corp 2", "audit-corp-2",
                                                "admin2@audit.com")));
                verifyUserEmail("admin2@audit.com");
                verifyUser("admin2@audit.com");
                Long t2Id = Objects.requireNonNull(
                                tenantRepository.findByWorkspaceSlug("audit-corp-2").orElseThrow().getId());
                String t2Token = login("admin2@audit.com", "password123", Objects.requireNonNull(r2.getCompanyCode()),
                                t2Id);
                ...
                mockMvc
                                .perform(
                                                post("/api/v1/tenant/products")
                                                                ...
                                                                .with(tenant(Objects
                                                                                .requireNonNull(String.valueOf(t1Id))))
                                                                .contentType(Objects.requireNonNull(
                                                                                MediaType.APPLICATION_JSON))
                                                                .content(Objects.requireNonNull(objectMapper
                                                                                .writeValueAsString(createReq))))
