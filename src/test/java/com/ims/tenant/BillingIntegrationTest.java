...
public class BillingIntegrationTest extends BaseIntegrationTest {
  ...
  void testInvoiceCreation() throws Exception {
    ...
    Long productId =
        objectMapper.readTree(prodResult.getResponse().getContentAsString()).get("id").asLong();

    // Add initial stock properly via service
    inventoryService.increaseStock(
        Long.valueOf(tenantIdStr), productId, 100, "Initial stock", testUserId);
    ...
    Long customerId =
        objectMapper.readTree(custResult.getResponse().getContentAsString()).get("id").asLong();
