...
public class BillingIntegrationTest extends BaseIntegrationTest {
        ...
        void testInvoiceCreation() throws Exception {
                ...
                mockMvc.perform(post("/api/v1/tenant/stock/in")
                                ...
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                                String.format("{\"productId\": %d, \"quantity\": 10, \"notes\": \"Stock for billing test\"}",
                                                                productId)))
                                .andDo(print())
                ...
                if (invResult.getResponse().getStatus() != 201) {
                        System.out.println("INVOICE CREATION FAILED. Response: "
                                        + invResult.getResponse().getContentAsString());
                }

                AssertionErrors.assertEquals("Expected 201 Created", 201,
                                invResult.getResponse().getStatus());
                ...
                mockMvc.perform(patch("/api/v1/tenant/invoices/" + invoiceId + "/status")
                                .header("Authorization", "Bearer " + token)
                                .header("X-Tenant-ID", tenantIdStr)
                                .contentType(MediaType.APPLICATION_JSON)
